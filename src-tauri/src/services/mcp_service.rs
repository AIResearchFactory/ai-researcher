...
impl McpService {
    ...
    pub async fn get_tools(&self) -> Result<Vec<McpTool>> {
        ...
        log::info!(
            "MCP Discovery: Checking {} configured servers",
            settings.mcp_servers.len()
        );

        for config in settings.mcp_servers.iter().filter(|s| s.enabled) {
            log::info!(
                "MCP Discovery: Fetching tools from enabled server '{}' ({})",
                config.name,
                config.id
            );
            match self.get_server_tools(config).await {
                ...
                Err(e) => log::warn!(
                    "MCP Discovery: Failed to get tools from {}: {}",
                    config.name,
                    e
                ),
            }
    ...
    async fn get_server_tools(&self, config: &McpServerConfig) -> Result<Vec<McpTool>> {
        ...
        let response = self
            .call_json_rpc(&mut server, "tools/list", json!({}))
            .await?;
    ...
    async fn initialize_server(&self, server: &mut McpServer) -> Result<()> {
        ...
        self.send_notification(server, "notifications/initialized", json!({}))
            .await?;
    ...
    async fn call_json_rpc(
        ...
        let child = server
            .child
            .as_mut()
            .ok_or_else(|| anyhow!("Server child not found"))?;
        let stdin = child
            .stdin
            .as_mut()
            .ok_or_else(|| anyhow!("Stdin not found"))?;
        let stdout = child
            .stdout
            .as_mut()
            .ok_or_else(|| anyhow!("Stdout not found"))?;
        ...
        while let Some(line) = reader.next_line().await? {
            ...
            let response: Value = match serde_json::from_str(&line) {
                ...
                Err(e) => {
                    log::debug!(
                        "MCP {}: Ignoring non-JSON/invalid output: {}. Error: {}",
                        server.config.id,
                        line,
                        e
                    );
