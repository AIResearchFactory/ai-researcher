...
async fn test_onboarding_installation_flow() {
    ...
    app_lib::directory::create_directory_structure(&app_data_path).await.expect("Failed to create structure");
    assert!(app_data_path.join("projects").exists());
    assert!(app_data_path.join("skills").exists());
    
    // Create default files
    app_lib::directory::create_default_files(&app_data_path).await.expect("Failed to create default files");
...
async fn test_onboarding_update_flow_preserves_gemini() {
    ...
    let backup_path = update_manager.backup_user_data().await.expect("Backup failed");
