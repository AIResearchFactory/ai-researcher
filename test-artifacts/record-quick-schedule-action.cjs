...
(async () => {
  ...
  await page.getByTitle('Schedule').first().click().catch(()=>{});
  await page.waitForTimeout(900);

  await page.screenshot({ path: `${outDir}/42-quick-schedule-action.png`, fullPage: true });

  const v = await page.video().path();
  await context.close();
  await browser.close();
  fs.copyFileSync(v, `${outDir}/quick-schedule-action-demo.webm`);
})();
